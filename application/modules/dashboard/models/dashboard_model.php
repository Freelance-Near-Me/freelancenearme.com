...
class Dashboard_model extends BaseModel {
	...
	$this->load->helper('date');
		...
		if($i==0){
			...
			if ($upd_user) {
				...
				$msg['message']= __('dashboard_editprofile_your_profile_updated_successfully','Your profile has been updated successfuly.')."<a class='close' data-dismiss='alert' aria-label='close'>×</a>";
   ...
   public function insertEscrow($data)
   {
	...
	public function getProjectStaticsShow($user_id='',$type=''){
		$data = array();
		if($type=='F'){
			$data['active_projects'] = $this->db->where("FIND_IN_SET('".$user_id."', bidder_id)!=",0)->where('status','P')->where("!FIND_IN_SET('$user_id',ended_contractor)!=",0)->count_all_results('projects');
			$data['completed_projects'] = $this->db->where("FIND_IN_SET('".$user_id."', bidder_id)!=",0)->where('status','C')->or_where("FIND_IN_SET('$user_id',ended_contractor)!=",0)->count_all_results('projects');
			
			
			$data['total_contest'] = $this->db->from('contest c')->join('contest_entry c_n', 'c_n.contest_id=c.contest_id')->where('c_n.user_id', $user_id)->get()->num_rows();
			
		} else {
			$data['open_projects'] = $this->db->where(array('user_id' => $user_id, 'status' => 'O'))->count_all_results('projects');
			$data['completed_projects'] = $this->db->where(array('user_id' => $user_id, 'status' => 'C'))->count_all_results('projects');
			
			$data['active_projects'] = $this->db->where(array('user_id' => $user_id, 'status' => 'P'))->count_all_results('projects');
			$data['cnl_projects'] = $this->db->where(array('user_id' => $user_id, 'status' => 'CNL'))->count_all_results('projects');
		}
		return $data;
	}
