...
<script src="<?php echo JS;?>bootstrap.validate.js"></script>--><script type="text/javascript" src="<?=JS?>ajaxfileupload.js"></script>
 <style>
.item.selected {
...
?>         
        ...
        <div class="form-group">        	
            	...
            	<label for="" class="control-label">Describe Your Project in Detail:</label>            	
...
?>
		...
		<div class="row form-group" id="hourly_budget" <?php if($job_details[0]['project_type']=="H"){ echo "style=display:block;";}else{echo "style=display:none;";}?>>        	
			<div class="col-6">
				<label for="" class="control-label">Minimum Budget</label>            	
			...
			<div class="col-6">
				<label for="" class="control-label">Maximum Budget</label>            	
			...
			<div class="login_form logi_width multi_freelancer" style="display: none;">
				...
				<input type="radio" name="multi_freelancer" class="acount-radio2" value="Y" checked="checked" onchange="getMulti(this.value)"> Yes  
				<input type="radio" name="multi_freelancer" class="acount-radio2" value="N" onchange="getMulti(this.value)"> No 
...
<option value="0#100" <?php if(isset($buget) && $buget=="0#100"){echo "selected='selected'";}?>>Less than <?php echo CURRENCY;?> 100</option>
...
<option value="other"  <?php if(isset($buget) && $fixed_budget=="Y"){echo "selected='selected'";}?>>Optional (Enter exact amount)</option>
            </select>
            <span id="budgetallError" class="error-msg13"></span>        
			
			<div id="budget_other" style="<?php if($fixed_budget=="N"){echo 'display: none';}?>">
            	<label for="" class="control-label">Budget Amount</label>            	
        		<input class="form-control fixed_budeget" id="fixed_budeget" type="text" name="fixed_budeget" value="<?php if($fixed_budget=="Y"){echo $job_details[0]['buget_max'];}else{ echo '0';}?>" placeholder="Budget Amount">
        	</div>
		...
		<div id="f_q_wapper">
			...
			<div class="qBx" id="qs_item_<?php echo $k+1;?>"><div class="input-group mb-3"><input type="text" class="form-control" placeholder="Question 1" name="questions[]" readonly="readonly" value="<?php echo $v['question'];?>" /><div class="input-group-append"><button type="button" class="btn btn-success save-btn" onclick="saveQs(<?php echo $k+1;?>)" style="display: none;">Save</button>&nbsp;<button type="button" class="btn btn-danger" onclick="removeQs(<?php echo $k+1;?>)">Remove</button></div></div></div>
			<?php } } ?>
			
			
		</div>
		<button class="btn btn-site" type="button" onclick="addQuestion()" id="add_qs_btn">Questions for freelancer <i class="zmdi zmdi-plus-circle-o"></i></button>
        ...
        <div class="form-group">        	        
                ...
                I want my project to be listed as a featured project. Featured projects attract more, higher quality 
bids. They appear prominently on the website.</label>
                  </div> 				 
		...
		<p><label> Project Title :</label><span id="p_title"></span></p>
		<p><label> Your Project in Detail:</label><span id="p_description"></span></p>
		<p><label> Category :</label><span id="p_category_id"></span></p>
		<p><label> Sub Category :</label><span id="p_subcategory_id"></span></p>
		<p><label> Skill :</label><span id="p_select2-search__field"></span></p>
		<p><label> Project Environment :</label><span id="p_environment"></span></p>
		<p><label> Project Visibility :</label><span id="p_visibility"></span></p>
		<p><label> Project Type :</label><span id="p_project_type"></span></p>
		</div>
        ...
        <a class="btn btn-web" href="<?php echo base_url('dashboard/myproject_client'); ?>">Cancel</a> 
 ...
 function movefile(evt){ 
        ...
        $.ajaxFileUpload({
            ...
            success: function (data){              
                ...
                $("#flist_div").append("<div class='flisttext' id='sp_"+data.msg.replace(".","")+"'>"+data.msg+"<i class='fa fa-trash' id='"+data.msg+"' onclick='removespan(this.id)' style='float: right;color:#f00;font-size:18px;cursor:pointer'></i></div>");
