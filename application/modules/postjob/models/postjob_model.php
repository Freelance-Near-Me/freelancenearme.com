		...
		elseif($project_type=='H'){    
		  ...
		  if($this->input->post('featured')=="Y"){
			...
			if(($this->input->post('project_type')=="F" && $balance<FIXED_RATE) || ($this->input->post('project_type')=="H" && $balance<HOURLY_RATE)){
				...
				$msg['errors'][$i]['message']=__('postjob_no_enough_fund','You do not have enough fund. Please <a href="'.base_url('myfinance').'" target="_blank">add fund.</a>');
				$i++;   
			...
			if($this->session->userdata('user'))
			{
				...
				if(($this->input->post('project_type')=="F" && $balance<FIXED_RATE) || ($this->input->post('project_type')=="H" && $balance<HOURLY_RATE))
				{
					$msg['location']=VPATH.'myfinance/';	
						...
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => FIXED_RATE, 'ref' => $ref , 'info' => 'Project featured fee'));
						
						$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => FIXED_RATE, 'ref' => $ref, 'info' => 'Project featured fee received'));
					...
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $user_wallet_id, 'debit' => HOURLY_RATE, 'ref' => $ref , 'info' => 'Project featured fee'));
					
					$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => PROFIT_WALLET, 'credit' => HOURLY_RATE, 'ref' => $ref, 'info' => 'Project featured fee received'));
