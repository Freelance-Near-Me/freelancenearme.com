...
class Findtalents_model extends BaseModel {
	...
	public function list_freelancer($srch_param=array() , $limit=0 , $offset=40 , $for_list=TRUE){
		...
		$this->db->select("DISTINCT $user.user_id,$user.fname,$user.lname,$user.membership_plan,$user.city,$user.country,$user.slogan,$user.logo,$user.overview,$user.hourly_rate", FALSE);

		$this->db->from('user');
		if(!empty($srch_param['skill_id']) || !empty($srch_param['sub_skill_id'])){		
		$this->db->join('new_user_skill' , "$user.user_id=$user_skill.user_id" , "LEFT");
		}		
...

   {
...

	{
		...
		return $this->db->get_where('projects',array('user_id'=>$user_id,'status'=>'O'))->result_array();	
