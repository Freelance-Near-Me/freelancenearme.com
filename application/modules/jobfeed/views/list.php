...
<p>hrs/week <a href="javascript:void(0);" data-toggle="modal" data-target="#myModal"><i class="fa fa-pencil"></i></a></p>
...
<div class="progress profile_progress">
  <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" aria-valuenow="<?php echo round($completeness);?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo round($completeness);?>%">
   <?php echo round($completeness);?> %
  </div>
</div>
<ul class="profile-list">
	<li><img src="<?php echo VPATH;?>assets/images/cuntryflag/<?php echo $flag;?>" alt="" />  &nbsp;<span><?php echo $city; ?>,</span> <?php echo $c; ?></li>
    <li><i class="zmdi zmdi-time"></i> Hourly Rate: <?php echo CURRENCY;?><?php echo $rate;?></li>
    <li><i class="zmdi zmdi-sign-in"></i> Last logged on:  <?php echo date('d M,Y',strtotime($ldate));?></li>
    ...
    <li><a href="<?php echo base_url('favourite'); ?>"><i class="zmdi zmdi-favorite"></i> Favourite Projects</a></li>
    <li><a href="<?php echo base_url('dashboard/profile_professional'); ?>"><i class="zmdi zmdi-account"></i> View my profile</a></li>
    <li><i class="zmdi zmdi-money"></i> Amount Earned: <?php echo CURRENCY;?> <?php echo $this->clientdetails_model->get_total_earning($curr_user[0]->user_id);?></li>
    <li><i class="zmdi zmdi-money"></i> Over <?php echo CURRENCY;?> <?php echo $this->clientdetails_model->get_total_expenditure($curr_user[0]->user_id);?> Total Spent</li>
...
<div class="input-group input-group-lg">
  <input type="text" class="form-control" id="srch" onkeyup="catdtls(this.id);" placeholder="Enter Job Name to Search">
  <span class="input-group-addon" id="basic-addon1"><button type="submit" class="btn btn-site"><i class="zmdi zmdi-search"></i> Search</button></span>
</div>
</form>
...
?>	
    ...
    <p class="bio">Looking for experienced OpenCart 2 Template developer to modify purchased commercial template to our needs. We have an OpenCart 2 websites, commercial templates and we need someone who will be able to adapt in a timely manner template's HTML/CSS/PHP code to our needs ... <a href="#">more</a></p>
    </div>        	
    <ul class="skills">    
    	<li>Skills: </li>
...
<h4 style="float:left"> <?php echo ucwords($val['title']);?> <br> </h4></a> 
<?php
...
<div class="addthis_sharing_toolbox" data-url="<?php echo VPATH;?>jobdetails/details/<?php echo $val['project_id'];?>" style="float: right;margin-top: 8px;"></div>

<ul style="float:left" class="search-job-content-minili">
<li><?php if($val['project_type']=='F'){echo "Fixed";} else{echo "Hourly";}?> Price: <b>Between <?php echo CURRENCY;?> <?php echo $val['buget_min'];?> and <?php echo CURRENCY;?> <?php echo $val['buget_max'];?> </b> </li>
...
<ul class="skills">
<li><b>Category:</b></li>
<li><a href="<?php echo VPATH;?>jobfeed/filterjob/<?php echo str_replace('&','_',$lnki);?>/<?php echo $ptype."/".$minb."/".$maxb."/".$post_with."/".$coun."/".$ct."/".$featured."/".$environment."/".$uid."/";?>"><?php echo $val['category'];?></a> </li>
</ul>
<p>
...
<img src="<?php echo VPATH;?>assets/images/cuntryflag/<?php echo $code;?>.png"> &nbsp;&nbsp;
 ...
 <!--<a style="float: right;" href="javascript:void(0)" data-reveal-id="exampleModal" onclick="setaction(<?php echo $val['project_id'];?>)">
  ...
  {
  ?>       
  <a href="<?php echo VPATH;?>jobdetails/details/<?php echo $val['project_id'];?>/<?php echo $this->auto_model->getcleanurl($val['title']);?>/" class="logbtn2">Select this job</a>       
    <?php
    }
	else
	{
	?>
    <a href="<?php echo VPATH;?>login?refer=jobdetails/details/<?php echo $val['project_id'];?>/<?php echo $this->auto_model->getcleanurl($val['title']);?>/" class="logbtn2">Select this job</a>
    <?php
	}
	?>     
</div>
...
<ul class="subdcribe-bar-left"><li>All Jobs (<?php echo $total;?> Results)</li><li class="rss margtop"><a title="" data-toggle="tooltip" style="cursor: pointer;" data-original-title="Rss" href="<?php echo VPATH;?>rssfeed/index/<?php echo str_replace('&','_',$cat);?>/<?php echo $ptype."/".$minb."/".$maxb."/".$post_with."/".$coun."/".$ct."/".$featured."/".$environment."/".$uid."/";?>" target="_blank"><img src="<?php echo ASSETS;?>images/rss_icon.png"></a></li></ul>
...
?>
<ul class="skills"><li><b>Category:</b></li> <li> <a href="<?php echo VPATH;?>jobfeed/filterjob/<?php echo str_replace('&','_',$lnki);?>/<?php echo $ptype."/".$minb."/".$maxb."/".$post_with."/".$coun."/".$ct."/".$featured."/".$environment."/".$uid."/";?>"><?php echo $val['category'];?></a> </li>
</ul>
<p>
                ...
                <div class="mainacount" id="login_frm">
                    ...
                    <div id="jbinvite">
                        <div class="invite_form" id="fname_div1">
                            <input type="text" value="" name="fname[]" id="fname" class="invite-input" placeholder="Friend's Name" >                                        
                            <div class="error-invite" id="fnameError"><?php echo form_error('fname');?></div> 
                        </div>
                        
                        <div class="invite_form" id="femail_div1">     
                            <img src="<?php echo ASSETS?>images/close-icon.png" style="float: right;margin-top: 10px;margin-bottom: -3px;position: absolute;right: 51px;" id="close1" onclick="removeMore(1)">
                            <input type="text" id="femail" name="femail[]" value="" class="invite-input" placeholder="Friend's Email" >                                        
                        ...
                        <div class="invite_form" id="femail_div2"> 
                            <img src="<?php echo ASSETS?>images/close-icon.png" style="float: right;margin-top: 10px;margin-bottom: -3px;position: absolute;right: 51px;" id="close2" onclick="removeMore(2)">
                            <input type="text" id="femail" name="femail[]" value="" class="invite-input" placeholder="Friend's Email" >                                        
                            <div class="error-invite" id="femailError"><?php echo form_error('femail');?></div>
                        </div>
                        
                        <div class="invite_form" id="fname_div3">
                            <img src="<?php echo ASSETS?>images/close-icon.png" style="float: right;margin-top: 10px;margin-bottom: -3px;position: absolute;right: 51px;" id="close3" onclick="removeMore(3)">
                            <input type="text" value="" name="fname[]" id="fname" class="invite-input" placeholder="Friend's Name" >                                        
                            <div class="error-invite" id="fnameError"><?php echo form_error('fname');?></div> 
                        </div>
                    ...
                    <div class="masg3">
                        <input type="button" class="btn-normal btn-color submit bottom-pad2 invitebott " value="Add More"  onClick="return addMore();">
                    </div>
                </div>  
                    
                <div class="invite_form" style="float: right;width: 14% !important;"> 
                    <div class="masg3">
                        <input type="submit" name="invite" class="btn-normal btn-color submit bottom-pad2 invitebott " value="Invite"  onClick="return invitecheck();">
                    </div>
                </div>    
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
          ...
          <div class="col-xs-12">
             ...
             <input type="number" name="available_week" class="form-control" value="<?php echo !(empty($available_hr)) ? number_format($available_hr) : '' ; ?>"/>
          </div>
          </div>          
