    ...
    ?>
        ...
        <div class="panel panel-default panel-details">
          <div class="panel-heading">
            <ul class="flex-list">              
				<li><span><i class="icon-feather-calendar"></i> <?php echo __('jobdetails_posted_on','Posted on'); ?></span><br /><?php echo date('d-M-Y' , strtotime($posted_on));?></li>
                <li><span><i class="fas fa-coins"></i> <?php echo __('jobdetails_budget','Budget'); ?></span><br /> <?php echo $buget;?></li>
                <li><span><i class="icon-feather-file"></i> <?php echo __('jobdetails_project_type','Project Type'); ?></span><br /> <?php echo $project[0]['project_type']=="F" ? __('jobdetails_fixed','Fixed') : __('jobdetails_hourly','Hourly'); ?></li>
                <li><span><i class="icon-feather-clock"></i> <?php echo __('jobdetails_time','Time'); ?></span><br /> <?php echo $dayleft;?> <?php echo __('jobdetails_days_left','days left'); ?></li>                  
            </ul>
          </div>
...
if(is_numeric($project[0]['sub_category'])){
	...
	switch($lang){
			...
			?>
            <p><?php echo  htmlentities($project[0]['description']);?></p>
           <!--<h4><?php echo __('jobdetails_category','Category'); ?>: <span><?php echo  $categoryName;  ?></span></h4>-->
            ...
            <h4><?php echo __('jobdetails_experience_level','Experience level'); ?>: <span><?php echo  !empty($exp_level_row['name']) ? $exp_level_row['name'] . " <i style='font-size: 13px;'> (  {$exp_level_row['description']}  ) </i>" : 'N/A';  ?></span></h4>
...
?>
            ...
            <div class="task-tags">
              ...
              <?php } } ?>
            </div>			<br>			<?php			if($project[0]['attachment']){			?>			<h4><?php echo __('jobdetails_attachment','Attachment'); ?>:</h4>			<?php			$attachment=explode(",",$project[0]['attachment']);			foreach($attachment as $akey=>$aval){			?>			<img src="<?php echo base_url('assets/postjob_upload').'/'.$aval?>" width='100px'>			<?php				}			}			?>
          </div>
        </div>
        ...
        <div class="panel panel-default" id="new_bid_panel" style="display:none;">
          <div class="panel-heading">
            <h4 class="b"><?php echo __('jobdetails_bid_project','Bid Project'); ?></h4>
          </div>
          <div class="panel-body">
            <form action="" method="post" id="bid_form">
              <?php if($project[0]['project_type']=='F'){ ?>
				...
				<div class="form-group">
					<p>
						<input class="magic-radio" name="payment_at" type="radio" id="payment_at_milestone" value="M" checked="">
						<label for="payment_at_milestone" style="display:inherit">By milestone</label>
					</p>
					<p>
					<input class="magic-radio" name="payment_at" type="radio" id="payment_at_project" value="P">
					...
					<div class="row row-10">
						<div class="col-sm-4">
							<label><b>Bid Amount</b></label>
                             <div class="form-group">
							 <input type="text" name="amount[]" class="with-border required" value="<?php echo !empty($revised_milestone[0]['amount']) ? $revised_milestone[0]['amount'] : '0'; ?>" onkeypress="return isNumberKey(event)" onblur="checkAmount(this)"/>
							 <input type="hidden" name="title[]" value="Project payment"/>
							 <input type="hidden" name="date[]"  value="0000-00-00" />
                             </div>
						</div>
						<div class="col-sm-4">
							<label class="d-block"><b>Fee</b></label>
							<p><?php echo SITE_COMMISSION; ?> % </p>
						</div>
						<div class="col-sm-4">
                            <label class="d-block"><b>You'll be paid</b></label>
                            <p><span id="project_pay_amount">0</span></p>
						</div>
					</div>
				</div>	
			...
			<span id="milestone_wrapper">
              ...
              <div class="row row-10" id="row_<?php echo $k;?>">
                <div class="col-sm-4 col-xs-12">
                  <div class="form-group">
                    ...
                    <input type="text" name="title[]"  class="with-border required" value="<?php echo !empty($v['title']) ? $v['title'] : '';?>"/>
                  </div>
                </div>
                <div class="col-sm-4 col-xs-12">
                  <div class="form-group">
                    ...
                    <input type="text" name="date[]" class="with-border datepicker required" value="<?php echo !empty($v['mpdate']) ? $v['mpdate'] : '';?>" />
                ...
                <div class="col-sm-3 col-xs-12">
                  <div class="form-group">
                    ...
                    <input type="text" name="amount[]" class="with-border required" value="<?php echo !empty($v['amount']) ? $v['amount'] : '';?>" onkeypress="return isNumberKey(event)" onblur="checkAmountByMilestone()"/>
                  </div>
                </div>
              ...
              <div class="row row-10">
                ...
                <div class="col-sm-4 col-xs-12">
                    <label for="date"><?php echo __('jobdetails_due_date','Due Date'); ?></label>
                    <div class="input-group">
                    <input type="text" name="date[]" class="with-border datepicker required" />
                    <span class="input-group-addon">
                        <span class="glyphicon glyphicon-calendar"></span>
                    </span>                    
                  </div>
                ...
                <div class="col-sm-3 col-xs-12">
                    <label for="amount"><?php echo __('jobdetails_amount','Amount'); ?></label>
                    <input type="text" name="amount[]" class="with-border required"  onkeypress="return isNumberKey(event)" onblur="checkAmountByMilestone()" />
                </div>
			  ...
			  <div class="row">
				<div class="col-sm-6">
					<label><b>Fee: </b></label> <?php echo SITE_COMMISSION; ?> % 
				</div>
				<div class="col-sm-6">
					<?php
					...
					?>
					<label><b>You'll be paid: </b></label> $ <span id="milestone_pay_amt"><?php echo round($net_amount, 2);?></span>
				</div>
			  </div>
              ...
              <div class="form-group">
                ...
                <input type="text" value="<?php if($revised_user){echo $revised_data[0]['bidder_amt'];}?>" name="amount[]" id="total_amount" class="with-border">
                ...
                <input type="number" value="<?php echo !empty($revised_data[0]['available_hr']) ? $revised_data[0]['available_hr'] : $project[0]['hr_per_week']; ?>" class="with-border" name="available_hr" max="<?php echo $project[0]['hr_per_week']; ?>">
              ...
              <div class="form-group">
                ...
                <select name="update_status" class="with-border">
		...
		if(count($skills) > 0){foreach($skills as $k => $v){ 
			if(in_array($v['id'], $u_skill_ids)){
              ...
              <div class="form-group">
                <label><b><?php echo __('jobdetails_cover_letter','Cover Letter'); ?></b> (**)</label>
                <textarea rows="3" name="details" id="details" class="cost_input with-border"><?php if($revised_user){echo $revised_data[0]['details'];}?></textarea>
				...
				<div>
					...
					<textarea rows="" cols="" name="freelancer_answer[]" class="with-border"><?php echo !empty($prev_ans['answer']) ? $prev_ans['answer'] : ''; ?></textarea>
				</div>	
				<?php } } ?>
              ...
              <div class="form-group">
                ...
                <input type="text" value="<?php if($revised_user){echo $revised_data[0]['days_required'];}?>" size="6" maxlength="3" name="required_days" id="required_days" class="with-border" onkeypress="return isNumberKey(event)">
			  ...
			  <p>(**) : <i>Will only visible to employer</i></p>
              <div class="form-group">
              	<div class="uploadButton margin-top-30">
                    <input class="uploadButton-input" type="file" accept="image/pdf/jpeg" name="userfile" onchange="movebidfile(this)" id="upload" />
        ...
        <ul class="nav nav-tabs" role="tablist">
          <li><a class="active" href="#all_propasal" aria-controls="all_propasal" role="tab" data-toggle="tab"><?php echo __('jobdetails_all_proposals','All Proposals'); ?></a></li>
		  <li><a href="#invited_users" aria-controls="shortlisted_proposal" role="tab" data-toggle="tab"><?php echo __('jobdetails_invited','Invited'); ?></a></li>
          <?php if(isset($uid) && $uid == $owner_id){ ?>
          <li style="display:none;"><a href="#hidden_proposal" aria-controls="hidden_proposal" role="tab" data-toggle="tab"><?php echo __('jobdetails_hidden','Hidden'); ?> </a></li>
          <li style="display:none;"><a href="#shortlisted_proposal" aria-controls="shortlisted_proposal" role="tab" data-toggle="tab"><?php echo __('jobdetails_shortlisted','Shortlisted'); ?></a></li>
          
          <?php } ?>
        </ul>
        <?php //} ?>
        <div class="tab-content">
          <div class="listing tab-pane fade show active" id="all_propasal" role="tabpanel">
            <?php 
...
?>
            <div class="listBox propal_<?php echo $row['id'];?>">                             
              <div class="media">
              <div class="media-left mr-3"> <a href="<?php echo VPATH;?>clientdetails/showdetails/<?php echo $row['bidder_id']?>">
                <?php 
  ...
  ?>
                <img src="<?php echo VPATH."assets/uploaded/".$bidder_details[0]['logo'];?>" alt="..." class="rounded-circle" height="96" width="96">
                <?php  
    } else{ 
  ?>
                <img src="<?php echo VPATH;?>assets/images/user.png" class="rounded-circle" height="96" width="96">
                <?php } ?>
                </a> 
              </div>
              <div class="media-body">
                <h4 class="media-heading"><a href="<?php echo VPATH;?>clientdetails/showdetails/<?php echo $row['bidder_id']?>"><?php echo $bidder_details[0]['fname']." ".$bidder_details[0]['lname'];?>
                  ...
                  <a href="<?php echo VPATH;?>message/browse/<?php echo $pid;?>/<?php echo $row['bidder_id'];?>"><i class="fa fa-comments yelC" style="font-size:24px;margin-left: 15px;"></i></a>
                  <?php } else{ ?>
                  <?php } } ?>
                  </a> </h4>
 ...
 ?>
                <p class="bio"><i class="icon-feather-map-pin"></i> <?php echo  $contry_info;?> <img src="<?php echo IMAGE;?>flags/<?php echo $flag;?>" alt="<?php echo $flag;?>" width="20" height="15" title=""></p>
...
if($project[0]['status']=="P" && in_array($row['bidder_id'],$bidder_id)) { ?>
                <img src="<?php echo VPATH;?>assets/images/awarded.png" alt="Award" title="<?php echo __('jobdetails_job_awarded','Job Awarded'); ?>" class="award float-right" />
                <?php 
					}
				?>
 ...
 <a href="<?php echo VPATH;?>message/index/0/<?php echo $row['bidder_id'];?>/<?php echo $pid;?>"><img src="<?php echo VPATH;?>assets/images/conversation.png" style="height:25px; width:25px;"></a><?php */?>
  ...
  if(isset($uid) && $uid==$owner_id){
   ...
   if($project[0]['status']=="O" || $project[0]['status']=="F" || ($project[0]['project_type'] == 'H' && $project[0]['no_of_freelancer'] > count($bidder_arr))) {  //get_print($bidder_arr, false); echo $row['bidder_id'];  ?>
                ...
                <button type="button" class="btn-edit hire_sec_<?=$row['bidder_id']?>" onclick="setFreelancer('<?php echo $row['bidder_id']?>', '<?php echo $bidder_details[0]['fname']." ".$bidder_details[0]['lname'];?>', this);" data-toggle="tooltip" data-escrow-enabled="<?php echo $row['enable_escrow']; ?>" data-placement="bottom" title="Award" data-href="<?php echo base_url('jobdetails/viewmilesone/'.$row['id']); ?>" data-bid-id="<?php echo $row['id']; ?>"><i class="zmdi zmdi-graduation-cap"></i></button>
...
if(isset($uid) && $uid==$owner_id){
    if($project[0]['project_type'] == 'F'){
        echo '<a href="#" data-toggle="modal" data-target="#milestone" class="btn-edit show_milestone" data-href="'.base_url('jobdetails/viewmilesone/'.$row['id']).'" data-bid-id="'.$row['id'].'"><i class="zmdi zmdi-flag" data-toggle="tooltip" data-placement="bottom" data-original-title="Milestone" title="'.__('jobdetails_milestone','Milestone').'"></i></a>';
    }
}
?>
                <?php 	
...
?>
                <?php if(isset($uid) && $uid == $owner_id){ ?>
                <span class="proposal_action_button" style='display: inline-block; vertical-align: middle;'> <a href="javascript:void(0);" class="round-button btn-del" data-toggle="tooltip" data-placement="bottom" title="<?php echo __('jobdetails_not_interest','Not interested'); ?>" onclick="moveToHidden('<?php echo $row['id'];?>', this)"><i class="zmdi zmdi-thumb-down"></i> </a> <a href="javascript:void(0);" class="round-button btn-aktive" data-toggle="tooltip" data-placement="bottom" title="<?php echo __('jobdetails_shortlist','Shortlist'); ?>" onclick="moveToShortlist('<?php echo $row['id'];?>', this)"><i class="zmdi zmdi-thumb-up"></i></a> </span>
                <?php } ?>
                <?php  //if(isset($uid) && $uid==$owner_id){ ?>
                <h5><b><?php echo __('jobdetails_bid_amount','Bid Amount'); ?>:</b> <?php echo CURRENCY . ' '.$row['bidder_amt'];?></h5>
                <?php //} ?>
                <ul class="display-inline m-0">
    			...
    			?>
                  ...
                  <li><?php echo __('jobdetails_job_success','Job Success'); ?> <strong><?php echo round($success_prjct);?>% </strong></li>
                  <li><?php echo CURRENCY;?> <b><?php echo $bidder_details[0]['hourly_rate'];?> / hr</b></li>
                  <li><?php echo __('jobdetails_compleated_projects','Completed Project'); ?>: <b><?php echo $count_project;?></b> </li>
                  <li><?php echo __('jobdetails_amount_earned','Amount Earned'); ?> <b><?php echo CURRENCY. ' '.$total_earning; ?></b> </li>
                </ul>
                
                <p><b><?php echo __('jobdetails_submit_on','Submit on'); ?>:</b> <?php echo date("d F Y",strtotime($row['add_date']));?> | <span> <b><?php echo __('jobdetails_duration','Duration'); ?>:</b> <?php echo $row['days_required'];?> <?php echo __('jobdetails_days','Days'); ?> </span> | <span><b><?php echo __('jobdetails_attachment','Attachment'); ?>:</b></span> | <b>Provide Update:</b> <?php echo !empty($row['update_status']) ? $row['update_status'] : 'N/A'; ?>
        ...
        ?>
                    <li><a href="<?php echo VPATH;?>findtalents/browse/<?php echo $this->auto_model->getcleanurl($s['parent_skill_name']).'/'.$s['parent_skill_id'].'/'.$this->auto_model->getcleanurl($s['skill']).'/'.$s['skill_id']?>"><?php echo $sname;?></a></li>
                    <?php
           }       
          }
          else{ 
			   ...
			   $contry_info.$user_det['country'];
				...
				<div class="media">
					<div class="media-left mr-3">
						<img class="media-object" src="<?php echo $pic; ?>" alt="" height="84" width="84">
					</div>
					<div class="media-body">
						 ...
						 <p class="bio"><i class="zmdi zmdi-map"></i> <?php echo  $contry_info;?> <img width="16" height="11" title="" src="<?php echo VPATH;?>assets/images/cuntryflag/<?php echo $flag;?>"></p>
					</div>
				</div>
				<?php } }else{ ?>
    ...
    <div class="col-md-3 col-sm-12 col-xs-12 dpl-0">    	
        ...
        <div class="left_sidebar">                  
          <div class="c_details white margin-bottom-20" style="border:1px solid #e1e1e1">
            <div class="profile">
	...
	?>
            ...
            <div class="profile-details text-center">
              ...
              <?php 
				...
				?>
              <p><img src="<?php echo ASSETS;?>images/cuntryflag/<?php echo $flag;?>" /> <span><?php echo $user[0]['city'];?></span> , <?php echo $c;?></p>
              <p><i class="zmdi zmdi-case"></i> <?php echo $user_totalproject;?> <?php echo __('jobdetails_job_posted','Jobs Posted'); ?></p>
              <p><i class="zmdi zmdi-assignment-check"></i> <span><?php echo $user[0]['com_project'];?> <?php echo __('jobdetails_completed_project','Completed Project'); ?></span></p>
              <p><i class="zmdi zmdi-money-box"></i> <?php echo CURRENCY;?><?php echo round(get_project_spend_amount($user[0]['user_id']));?> <?php echo __('jobdetails_total_spent','Total Spent'); ?></p>
 ...
 <div class="clearfix">
				...
				<?php /* <a href="#" class="btn btn-site btn-block" style="margin-bottom:12px" id="bid_now_button"><i class="fa fa-gavel"></i> <?php echo strtoupper(__('Bid Now','BID NOW')); ?></a> */ ?>
				...
				if($verify_new == 'N'){
					?>
					<button class="btn btn-site btn-block" style="margin-bottom:12px" id="bid_no_button" data-toggle="modal" data-target="#no_bid"><i class="fa fa-gavel"></i> <?php echo strtoupper(__('jobdetails_bid_now','BID NOW')); ?></button>
					<?php
				}
				else{
					?>
					<a href="#" class="btn btn-site btn-block" style="margin-bottom:12px" id="bid_now_button"><i class="fa fa-gavel"></i> <?php echo strtoupper(__('jobdetails_bid_now','BID NOW')); ?></a>
					<?php
				}
				?>
              ...
              <a href="#" class="btn btn-site btn-block" style="margin-bottom:12px" id="revised_bid_button"><i class="fa fa-gavel"></i> <?php echo strtoupper(__('jobdetails_revised_bid','REVISED BID')); ?></a>
              <?php } }  ?>
           
				<?php } ?>
				
              <?php if(!$uid){ ?>
              <a href="<?php echo base_url('login');?>" class="btn btn-site btn-block"><i class="fa fa-user-plus"></i> <?php echo strtoupper(__('jobdetails_invite_friend','INVITE FRIEND')); ?></a>
              <?php }else{ if($user[0]['user_id']==$uid){  ?>
              <a href="#" class="btn btn-site btn-block margin-bottom-20" data-toggle="modal" data-target="#inviteModal"><i class="fa fa-user-plus"></i> <?php echo __('jobdetails_invite_friend','Invite Friend'); ?></a>
              <?php } } ?>
		...
		<div class="modal-body">
		  <div class="row">
			<div class="col m12 s12">
				<h4 style="margin-top: 30px; text-align: center;"><?php echo __('you_cant_bid_on_job_until_your_account_has_not_verified_by_admin','You can\'t bid on job until your account has not verified by admin')?>.</h4>
			</div>
		  </div>
		</div>
  ...
  <div class="modal-content" style="border-radius:0px;">
	...
	<div class="modal-footer">
	 <?php if($project['status'] == 'O'){  ?>
	  <button type="button" class="btn btn-site" data-toggle="modal" data-target="#requestnewmilestone" onclick="$('#milestone').modal('hide');">Request New Milestone</button>
	  <?php } ?>
...
<div class="modal-dialog"> 
  ...
  <div class="modal-content" style="border-radius:0px;">
	<div class="modal-header">
    <h4 class="modal-title">Change Milestone Request</h4>
...
<div class="modal-dialog"> 
  ...
  <div class="modal-content" style="border-radius:0px;">
	<form action="" method="post" id="request_new_milestone_form">
	  <div class="modal-header">
      <h4 class="modal-title">Request New Milestone</h4>
	  ...
	  <div class="modal-body" id="request_milestone_body">
		...
		<textarea class="with-border" placeholder="Enter your comment here to request new milestone.." name="client_comment"></textarea>
	  </div>
	  <div class="modal-footer">
...
<div id="hireFreelancer" class="modal fade" role="dialog">
  <div class="modal-dialog"> 
    ...
    <div class="modal-content">
      <div class="modal-header">
      <h4 class="modal-title">Award Freelancer</h4>
...
<div class="modal fade" id="inviteModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      <h4 class="modal-title"><?php echo __('jobdetails_invite_friends_to_this_project','Invite Friends To this Project'); ?></h4>
      ...
      <div class="modal-body">
        <form  id="invite_user_form" method="post" class="form-horizontal">
          ...
          <div class="ui fluid selection dropdown">
            ...
            <input type="text" class="with-border" placeholder="<?php echo __('jobdetails_search_user_by_name_or_email','Search user by name or email'); ?>" onkeyup="search_freelancer(this.value)"/>
          ...
          <div class="text-right">
            <button type="button" class="btn btn-site" id="invite_user_button" style="margin-top:20px"><?php echo __('jobdetails_invite','Invite'); ?></button>
          </div>
        </form>
      </div>
    </div>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      <h4 class="modal-title" id="myModalLabel">My Bid</h4>
            ...
            <div class="cost_timing">
              <div class="form-group">
                <div class="col-xs-12">
                  ...
                  <textarea rows="" cols="" name="details" id="details" class="with-border"><?php if($revised_user){echo $revised_data[0]['details'];}?></textarea>
              ...
              <div class="form-group">
                <div class="col-xs-12">
                  <label>My Bid</label>
                  <input type="text" onkeypress="return isNumberKey(event)" onblur="getbid(this.value);" value="<?php if($revised_user){echo $revised_data[0]['bidder_amt'];}?>" size="6" maxlength="6" id="client_bidamount" name="bidder_amt" class="with-border">
                  <span id="client_bidamountError" class="rerror" style="float: left;"></span> <span style="float: left;">Note: If the project pays hourly enter hourly rate here, otherwise enter total charge</span> </div>
              </div>
              <!--   <div class="form-group">
                    <h1><b>+ <?php echo $bidwin_charge;?>% Jobbid Fee : <span id="comission">$</span></b></h1>
                    <input type="text" readonly value="" size="3" maxlength="3" id="comission" name="comission" class="with-border">                   
                </div>-->
              <div class="form-group" style="display:none;">
                <div class="col-xs-12">
                  <label>Charged to Client </label>
                  <input type="text" readonly value="<?php if(isset($uid) && $revised_user){echo $revised_data[0]['total_amt'];}?>" size="6" maxlength="6" id="bidamount" name="total_amt" class="with-border">
                </div>
              </div>
              <?php if($project[0]['project_type']=='F'){ ?>
              <div class="form-group">
                <div class="col-xs-12">
                  ...
                  <input type="text" value="<?php if($revised_user){echo $revised_data[0]['days_required'];}?>" size="6" maxlength="3" name="days_required" id="delivery" class="with-border">
                  <span id="deliveryError" class="rerror" style="float: left;"></span> </div>
              </div>
              <?php
				  }
              ...
              <div class="form-group">
                <div class="col-xs-12">
                  <div class="input-group">
                    ...
                    <input type="text" class="with-border" readonly>
                  </div>
                </div>
              </div>
              <!--<div class="form-group">
...
<script>
	...
	jQuery(document).ready(function($){
		...
		$('#add_more_milestone').click(function(){
			var html = '<div class="row row-10" id="row_'+count_row+'"><div class="col-sm-4 col-xs-12"><div class="form-group"><label for="title">Milestone Title</label><input type="text" name="title[]"  class="with-border required"/></div></div><div class="col-sm-4 col-xs-12"><div class="form-group"><label for="date">Due Date</label><div class="input-group datepicker"><input type="text" name="date[]" class="with-border datepicker required" /><span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span></div></div></div><div class="col-sm-3 col-xs-12"><div class="form-group"><label for="amount">Amount</label><input type="text" name="amount[]" class="with-border required" onkeypress="return isNumberKey(event)" onblur="checkAmountByMilestone()"/></div></div><div class="col-sm-1"><label style="display: block">&nbsp;</label><a href="javascript:void(0);" class="close_milestone" data-close="row_'+count_row+'"><i class="zmdi zmdi-close-circle"></i></a></div></div>';
	...
	function moveToShortlist(proposal_id, ele){
		var $ = jQuery;
		var p = $('#all_propasal .propal_'+proposal_id).clone();
		p.find('.proposal_action_button').html('<a href="javascript:void(0);" class="btn-aktive round-button active" title="Unshortlist" onclick="removeFromShortlist(\''+proposal_id+'\', this)" data-toggle="tooltip" data-placement="bottom" title="Unshortlist"><i class="zmdi zmdi-thumb-up" aria-hidden="true"></i></a>');
...
//document.getElementById('main').click(alert("HH"));
 ...
 function addMore(){ 
   ...
   var v="<div class='form-group'><div class='invite_form' id='fname_div"+c+"'><div class=\"col-sm-6 col-xs-12\"><input type='text' value='' name='fname[]' id='fname' class='with-border' placeholder='Friend&rsquo;s Name' ><div class='error-invite' id='fnameError'><?php echo form_error('fname');?></div></div></div><div class=\"col-sm-6 col-xs-12\"><div class='invite_form' id='femail_div"+c+"'><img src='<?php echo ASSETS?>images/close-icon.png' style='float: right;margin-top: 10px;margin-bottom: -3px;position: absolute;right: 30px;' id='close"+c+"' onclick='removeMore("+c+")'><input type='text' id='femail' name='femail[]' value='' class='with-border' placeholder='Friend&rsquo;s Email' ><div class='error-invite' id='femailError'><?php echo form_error('femail');?></div></div></div></div>"; 
  $("#jbinvite").append(v);
}
...
function checkAmount(ele){
	...
	var to_pay = val - comm;
	$('#project_pay_amount').html('$'+to_pay.toFixed(2));
