	...
	if($this->input->post()){
		...
		if($insert){
			...
			if($lst_msg){
                ...
                $resp['time']=date('h:i A',strtotime($d));
                if($post_data["recipient_id"] != 'room') {
...
public function unread_msg(){
	...
	if(count($unseen_msg) > 0){
		...
		$data['unseen_msg'] = $unseen_msg;
		$data['user_id'] = $user_id;
		$data['project_id'] = $project_id;
		$data['sender_id'] = $sender_id;
		$data['sendID'] = $sendID;
		$data['user'] = $user;
		$this->load->view('msg_unseen', $data);
...
public function get_left_msg($type=''){
	$counter = 0;
	$user=$this->session->userdata('user');
	$user_id=$user[0]->user_id;
	...
	if(count($message) > 0){
		foreach($message as $k => $v){
			...
			echo '<div class="messagtext2" id="msg_project_'.$v['project_id'].'_u_'.$v['sender_user'].'">';
			if($v['unread_msg']>0){
...
public function detailsmsg($pid='',$sid='',$limit_from='',$interview_status=''){
	...
	$this->load->view('msg_details', $data);
...
public function send_attachment(){
	...
	if(post()){
		...
		if(!empty($_FILES['attachment']['name'])){
			...
			if(!is_dir('assets/question_file')){
				mkdir('assets/question_file');
			}
		...
		$lst_msg = get_row(array('select' => '*', 'from' => 'message', 'where' => array('id' => $message_id)));
		...
		if($message_id){
			...
			if($post["recipient_id"] != 'room'){
				$count = file_get_contents($dir."user_".$post["recipient_id"].".newmsg");
				if($count > 0){
					$count += 1;
				}else{
					$count = 1;
				}
				file_put_contents($dir."user_".$post["recipient_id"].".newmsg" , $count);
			}
		}
		
		$d = $lst_msg['add_date'];
		$json['status'] = 1;
		$json['last_message'] = $lst_msg;
		
		$json['message']=$lst_msg['message'];
		$json['attach']=$lst_msg['attachment'];
		$json['date']=ucwords(date('d M,Y H:i:s',strtotime($d)));
		$json['time']=date('h:i A',strtotime($d));
		$json['add_fav']=$lst_msg['id'];
