			...
			if($google_user_id){
				...
				if($count_google_user > 0){
					...
					if($result){
						...
						if($result[0]->account_type=='' && $result[0]->token!=''){
							$this->session->unset_userdata('user');
							$this->session->set_userdata('pre_user', $result);
							$json['next'] = base_url('user/set_account_type/'.$result[0]->token);
						}elseif($result[0]->account_type=='F' && ($result[0]->verify=='N')){
							$this->session->set_userdata('FIRSTLOGIN', 'Y');
							$json['next'] = VPATH.'dashboard/profile_professional';
						}
					}
		...
		if(post()){
			...
			if($google_user_id){
				...
				if($count_google_user > 0){
					...
					$this->db->select('user_id, username, email,ldate,membership_plan,membership_end,membership_upgrade,acc_balance,account_type,verify,token')
					->from('user')
					->where(array('email' => post('email'), 'status'=>'Y'));
