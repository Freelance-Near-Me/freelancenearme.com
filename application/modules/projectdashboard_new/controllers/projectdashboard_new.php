...
class Projectdashboard_new extends MX_Controller {
	...
	private function freelancer_overview($project_id=''){
		...
		$breadcrumb=array(
			array(
					'title'=>'Project overview','path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,'Project overview');
	...
	private function freelancer_milestone($project_id=''){
		...
		$breadcrumb=array(
			array(
					'title'=>'Project milestone','path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,'Project milestone');
	...
	private function employer_milestone($project_id=''){
		...
		if($project_type == 'H'){
			...
			$data['outgoint_milestone_list']=$this->projectdashboard_model->getOutgoingMilestone($data['user_id'],$project_id);
			$data['incoming_milestone_list']=$this->projectdashboard_model->getIncomingMilestone($data['user_id'],$project_id);
		...
		if(post() && $this->input->is_ajax_request()){
			...
			if($this->form_validation->run()){
				...
				if(!empty($post['request_id'])){
					...
					$notification = "$freelancer updated project start date for $project_title";
					...
					$notification = "$freelancer request a project start date for $project_title";
					$link = 'projectroom/employer/overview/'.$project_id;
					$this->notification_model->log($user_id, $project_user, $notification, $link);
	...
	public function release_manual_hour(){
		...
		if(post() && $this->input->is_ajax_request() && $user_id){
			...
			if($error == 0){
				...
				$this->transaction_model->add_transaction_row(array('txn_id' => $new_txn_id, 'wallet_id' => $freelancer_wallet_id, 'credit' => ($total_cost_new+$commission), 'ref' => $ref , 'info' => 'Project payment received #'.$project_id));
	...
	public function create_invoice_hourly(){
		...
		if($this->input->is_ajax_request() && $user){
			...
			$notification = "{$user_info['fname']} send you a invoice for $projects_title";
	...
	public function invoices($project_id='', $limit_from=0){
		...
		$breadcrumb=array(
			array(
					'title'=>'Project Invoice','path'=>''
			)
		);
		$data['breadcrumb']=$this->autoload_model->breadcrumb($breadcrumb,'Project Invoice');
		...
		if($this->input->is_ajax_request() && $user){
			...
			if($cmd == 'accept'){
				...
				if($json['status'] == 1){
					...
					$notification = "Invoice number #$invoice_number has been paid";
					$link = 'projectroom/invoices/'.$project_id;
			...
			if($cmd == 'deny'){
				...
				if($json['status'] == 1){
					...
					$notification = "Invoice number #$invoice_number has been rejected";
					$link = 'projectroom/invoices/'.$project_id;
					$this->notification_model->log($user[0]->user_id, $freelancer_id, $notification, $link);
