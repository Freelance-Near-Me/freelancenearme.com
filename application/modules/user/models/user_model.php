...
class user_model extends BaseModel {
	...
	public function count_project($perent){
		...
		$this->db->where("skills.cat_id" , $perent);
		$this->db->where(array("projects.status"=>'O',"projects.project_status"=>'Y'));
		$this->db->where("projects.visibility_mode", "Public");
	...
	public function getNo_of_user(){
		$result = $this->db->select('no_of_users,no_of_projects,no_of_completed_prolects')->from('setting')->where('id',1)->get()->result_array();
		return $result;
	}
}
