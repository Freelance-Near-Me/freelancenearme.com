...
class User extends MX_Controller {
    ...
    public function index() {
		...
		$data['no_of_user'] = $this->user_model->getNo_of_user();
	...
	public function set_account_type($token=''){
		$data['token'] = $token;
		$this->load->view('set_account_type',$data);
	}
	
	public function acc_type_update(){
		$user = $this->session->userdata('pre_user');
		$user_id = $user[0]->user_id;
		$data = $json = array();
		$token = post('token');
		if(post() && $user_id && !empty($token)){
			$data = post();
			$count = $this->db->where('token', $token)->get('user')->row_array();
			if($count['user_id'] == $user_id){
				$upd = $this->user_model->acc_type_update_model($data);
			}
	...
	public function migrate(){
		...
		$except = array('demoemployer,demoemployee1,demoemployee2');
		$users = $this->db->select('user_id,username')->get('user')->result_array();
		if($users){
			foreach($users as $k => $v){
				...
				$update = array(
					'fname' => 'Demo',
					'lname' => 'User '.$u_key,
					'email' => 'demouser'.$u_key.'@gmail.com',
					'username' => 'demouser'.$u_key,
				
				);
				
				if(in_array($v['username'], $except)){
					continue;
				}
				
				$this->db->where('user_id', $v['user_id'])->update('user', $update);
			}
		}
	...
	public function checkmail(){
		$to="smritimaydebnath@gmail.com";
		$subject="This is a mail ".time();
		$contents="new content date time".time();
		$this->load->library('email');
		$this->email->initialize(array(
              'protocol' => get_option_value('protocol'),
              'smtp_host' => get_option_value('smtp_host'),
              'smtp_user' => get_option_value('smtp_user'),
              'smtp_pass' => get_option_value('smtp_pass'),
              'smtp_port' => get_option_value('smtp_port'),
              'mailtype' => get_option_value('mailtype'),
			  'charset'   => get_option_value('charset')
           ));
        $this->email->from('admin@skillancer.com', 'Skillancer');
        $this->email->to($to);
