...
<div class="col-lg-3 col-md-4 col-sm-12" style="border-right:1px solid #e0e0e0; min-height:520px">
    <div class="left_sidebar">
        ...
        <div class="profile">
            ...
            <?php if($verify=='Y'){ ?> <a class="btn- approved" style="opacity:1;border-radius:15px" title="<?php echo strtoupper(__('clientdetails_sidebar_approved','APPROVED')); ?>"><i class="zmdi zmdi-thumb-up"></i></a> <?php }  ?>
            </div>
		...
		<?php  if($user_id==$user_login) { ?><a href="<?php echo base_url('dashboard/editprofile_professional')?>" class="pull-right"><i class="icon-feather-edit-2"></i> <?php echo __('clientdetails_sidebar_edit','Edit'); ?></a> <?php } ?>
		
		<?php }  ?> 
		
        <h4 class="text-center"><?php echo $fname." ".$lname;?></h4>
		<p class="text-center"><span><?php echo $slogan?></span></p>
        <?php /*?><h4 class="text-center">
		<?php
		...
		?>
		<div class="star-rating mb-3 d-block" data-rating="<?php echo $avg_rating?>" style="max-width:150px; margin:0 auto"></div>
		<?php 
   ...
   $c = getField('Name', 'country', 'Code',$country);
    ...
    <ul class="profile-list">
        ...
        <li><img class="flag" src="<?php echo IMAGE;?>flags/<?php echo $flag;?>" alt=""> <?php echo $city; ?>, <?php echo $c; ?></li>
		 ...
		 <li><i class="icon-line-awesome-sign-out"></i> <?php echo __('clientdetails_sidebar_last_logged_on','Last logged on'); ?>: <?php echo date('d M,Y',strtotime($ldate));?></li>
		...
		<?php } } ?>
		 ...
		 <li><i class="icon-feather-tag"></i> <?php echo __('clientdetails_sidebar_total_spent','Total Spent'); ?>: <?php echo CURRENCY;?><?php echo get_project_spend_amount($user_id);?></li>
		<?php  } ?>
		
		
		<?php if($account_type == 'F'){ ?>
        <li><i class="zmdi zmdi-time"></i> <?php echo __('clientdetails_sidebar_hourly_rate','Hourly Rate'); ?>: <?php echo CURRENCY;?><?php echo $rate;?></li>
	...
	?>  
        <a class="btn btn-site btn-block" data-toggle="modal" data-target="#myModal2" onclick="setProject2('<?php echo $user_id;?>','<?php echo $user_login;?>')"><i class="fa fa-user-plus"></i> <?php echo __('clientdetails_sidebar_send_message','Send Message'); ?></a>
       ...
       <input type="hidden" name="freelancer_id2" id="freelancer_id2" value=""/>   
            ...
            <div class="cost_timing" id="message_wrapper">
                <div class="cost_form_box">                    
                    ...
                    <span id="detailsError" class="rerror" style="float: left; display:none; color:red"><?php echo __('clientdetails_sidebar_enter_your_message_first','Enter Your Message First'); ?></span>
                </div>
