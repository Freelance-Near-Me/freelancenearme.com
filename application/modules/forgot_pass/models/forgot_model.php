...
class Forgot_model extends BaseModel {
	...
	public function check_email() {
		...
		if($i==0){
			...
			if ($query->num_rows() == 1) {
				$token = md5(time().'-'.$result[0]->user_id);
				$this->db->where('user_id', $result[0]->user_id)->update('user', array('email_verification_link' => $token));
				//$nurl=VPATH."forgot_pass/reset_pass/".base64_encode($result[0]->user_id);
				$nurl=VPATH."forgot_pass/reset_pass/".$token;
				...
				$msg['status']='OK';
				//$msg['url']=$nurl;
				$msg['message']=__('forgotpass_new_password_has_been_sent_to_your_email','New password was sent to your email.');
	...
	public function reset_password() {
		...
		if($i==0){
			...
			$this->db->where('email_verification_link', $user_id);
			...
			if ($query) {
				...
				$msg['message']=__('forgotpass_password_reset_successful','Password Reset Successfully.').' '.__('forgotpass_please','Please').' <a href="'.VPATH.'login" style="color:#fff; border-bottom: 1px solid;">'.__('forgotpass_login_in','Log In').'</a>'.' '.__('forgotpass_to_continue','To Continue.');
