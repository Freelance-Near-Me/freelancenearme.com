...
if(!function_exists('get_country_currency')){
	function get_country_currency($code=''){
		$currency = 'USD';
		$country_curr = getField('currency_code', 'country', 'Code', $code);
		if($country_curr){
			$currency = $country_curr;
		}
		return $currency;
	}
}
...
if(!function_exists('get_user_currency')){
	function get_user_currency($user_id=''){
		$code = getField('country', 'user', 'user_id', $user_id);
		$currency = get_country_currency($code);
		return $currency;
	}
}

if(!function_exists('get_option_value')){
	function get_option_value($option_key){
		$setting_key = getField('setting_value', 'setting_option', 'setting_key', $option_key);
		return $setting_key;
	}
}if(!function_exists('getAllCategoryData')){	function getAllCategoryData(){		$ci = &get_instance();		$data = $ci->db->select('*')				->from('categories')				->where(array('parent_id'=>0, 'status'=>'Y'))				->get()->result_array();		return $data;	}}