...
$(document).ready(function() {
	...
	if($(".chart").length) {
		$(function () {
			...
			for($i=0;$i<=7;$i++)
			{
				...
				$new=$re[0]."-".$re[1]."-".$re[2];
			?>
				d1.push([<?=strtotime($new)?>,<?=$tot_g[$i]?>]);
				d2.push([<?=strtotime($new)?>,<?=$tot_f[$i]?>]);
			<?php }?>
	...
	if($(".chart-pie-social").length) {
		$(function () {
			...
			var data = [
			    { label: "Genuine",  data: <?php echo $alg;?>, color: chartColours[0]},
			    { label: "Expired",  data: <?php echo $ale;?>, color: chartColours[1]},
			    { label: "Fake",  data: <?php echo $alf;?>, color: chartColours[2]}
			];
