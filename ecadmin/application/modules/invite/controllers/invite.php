...
class Invite extends MX_Controller {
    ...
    public function delete() {
        ...
        $delete = $this->invite_model->delete_invite($id);

        if ($delete) {
            $this->session->set_flashdata('succ_msg', 'Invitee Deleted Successfully');
        } else {
	...
	public function page($limit_from='')
	{
        ...
        $config = array();
        $config["base_url"] = base_url() . "invite/page";
        $config["total_rows"] = $this->db->get('invite_friend')->num_rows();
        $config["per_page"] = 20;
        ...
        $data['list'] = $this->invite_model->getFriendList($config['per_page'],$start);
	...
	public function registered($limit_from='')
	{
        ...
        $data['list'] = $this->invite_model->getRegisteredList($config['per_page'],$start);
        //$data['edit'] = $this->notification_model->update_countrymenu();
