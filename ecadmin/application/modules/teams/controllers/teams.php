...
class Teams extends MX_Controller {
    ...
    public function index() {
		...
		$data['all_data'] = $this->teams_model->getAllTeamList();
	...
	public function add()
	{
		...
		$data['data'] = $this->auto_model->leftPannel();
		$lay['lft'] = "inc/section_left";
		$data['desig']=$this->teams_model->getDesignation();
				
		if($this->input->post('submit'))
		{
            $config['upload_path'] = '../assets/team_image/';
			...
			$configs['source_image']	= '../assets/team_image/'.$image;
	...
	public function edit()
	{
		...
		if($id)
		{
			$data['all_data'] = $this->teams_model->getAllTeamListbyId($id);
			$data['desig']=$this->teams_model->getDesignation();
		...
		if($this->input->post('submit'))
		{
            ...
            if ($this->form_validation->run() == FALSE) {
				...
				$new_data['role'] = $this->input->post('role');
				...
				if($image!='')
				{
					...
					$newnm=$newnm.".".$ext;
					unlink('../assets/team_image/'.$this->input->post('currimg'));
					unlink('../assets/team_image/'.$newnm);
				}
			$post_data['id'] = $id;
			$update = $this->teams_model->updateTeam($new_data,$id);
			if ($update) {
				$this->session->set_flashdata('succ_msg', 'Update Successfully');
			} else {
				$this->session->set_flashdata('error_msg', 'unable to Update');
			}
			redirect(base_url() . 'teams/edit/'.$id);
			}
	...
	public function change_status()
	{
		...
		{
			$update = $this->teams_model->deleteTeam($id);	
		}
		else
		{
			$update = $this->teams_model->updateTeam($data,$id);
		}
