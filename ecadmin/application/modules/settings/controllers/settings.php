...
class Settings extends MX_Controller {
	...
	public function transfer_edit(){
		...
		if ($this->input->post()) {
			...
			$this->form_validation->set_rules('stripe_mode', 'Stripe Mode','required');
	...
	public function api_setting(){
		...
		if(post()){
			$service_data = post('service');
			if($service_data){
				foreach($service_data as $service => $params){
					if(count($params) > 0){
						foreach($params as $k => $v){
							$this->db->where(array('service_id' => $service, 'service_key' => $k))->update('api_setting', array('service_value' => $v));
						}
					}
				}
			}
		...
		$data['api_services'] = get_results(array('select' => '*', 'from' => 'api_service'));
		$this->layout->view('api_setting', $lay, $data,'setting');
	}
	...
	public function addServiceField(){
		...
		if(post() && $this->input->is_ajax_request()){
			$post = post();
			$check = $this->db->where(array('service_key' => $post['service_key'], 'service_id' => $post['service_id']))->get('api_setting')->num_rows();
			if($check == 0){
				$this->db->insert('api_setting', $post);
				$json['status'] = 1;
			}else{
				$json['status'] = 0;
				$json['msg'] = 'Field already exist';
			}
