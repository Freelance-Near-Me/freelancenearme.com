...
class Partner extends MX_Controller {
	...
	public function add()
	{
		...
		if($this->input->post('submit'))
		{
			...
			$this->form_validation->set_rules('description', 'Description', 'required');			$this->form_validation->set_rules('userfile', 'Image', 'required');
            if ($this->form_validation->run() == FALSE) {
                $this->layout->view('add', $lay, $data);
            } else {
			   	$post_data['name'] = $this->input->post('name');
				$post_data['url'] = $this->input->post('url');
				$post_data['description'] = $this->input->post('description');				$post_data['image'] = $this->input->post('userfile');
	...
	public function edit()
	{
		...
		if($this->input->post('submit'))
		{
			...
			$this->form_validation->set_rules('userfile', 'Image', 'required');			/* get_print($this->input->post()); */
            if ($this->form_validation->run() == FALSE) {
                $this->layout->view('edit', $lay, $data);
            } else {				
				$new_data['name'] = $this->input->post('name');
				$new_data['url'] = $this->input->post('url');
				$new_data['status'] = $this->input->post('status');				$new_data['description'] = $this->input->post('description');				$new_data['image'] = $this->input->post('userfile');
  ...
  public function sub_footer_list() {
	...
	public function edit_sub_footer()
	{
    	...
    	public function upload_file_ajax(){		$json = array();				if($this->input->is_ajax_request()){						if(!empty($_FILES['file']['name'])){				$config['upload_path']   = '../assets/partner_image'; 				$config['allowed_types'] = 'gif|jpg|png|jpeg|mp4|ogg|ogv|wmv';				$config['encrypt_name']  =TRUE;				$this->load->library('upload', $config);				if ( ! $this->upload->do_upload('file')) {					$json['error'] = $this->upload->display_errors();					$json['status'] = 0;				}else{					$upload_data = $this->upload->data();					/* get_print($upload_data); */					$src = str_replace($this->input->server('DOCUMENT_ROOT') ,  'http://'.$this->input->server('HTTP_HOST') , $upload_data['full_path']);										$json['file_url'] = $src;					$json['file_name'] = $upload_data['file_name'];					$json['status'] = 1;				}			}else{				$json['status'] = 0;				$json['error'] = 'Choose file first';			}					}else{			$json['status'] = 0;			$json['error'] = 'Invalid request';		}				echo json_encode($json);		}
