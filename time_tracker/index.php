...
//$ACT = isset($R_Q['action']) ? $R_Q['action'] : "login";
...
if ($ACT == "appsetting") {
    ...
    $XML = '<?xml version="1.0" encoding="utf-16"?>
                        <List>
                         <appsetting>
                                <forgot>https://www.freelancenearme.com/forgot_pass</forgot>
                                <about>https://www.freelancenearme.com/information/info/about_us/</about>
                                <newaccount>https://www.freelancenearme.com</newaccount>
                                <help>https://www.freelancenearme.com/information/info/how_it_works/</help>
                                <watermark>Freelance Near Me</watermark>
                         </appsetting>
                        </List>';
    ob_start();
	 ob_clean();
...
//========End appsetting ========//
function getEmployerName($con,$id) {
    $r = mysqli_fetch_array(mysqli_query($con,"select username,fname,lname from serv_user where user_id='" . $id . "'"));
...
function getProjectID($con,$id) {
    $r = mysqli_fetch_assoc(mysqli_query($con,"select project_id from serv_project_tracker where id=(select project_tracker_id from serv_project_tracker_snap where id=" . $id . ")"));
    return $r['project_id'];
}
...
if ($ACT == "login") {
   ...
   $sql = "SELECT * FROM  serv_user WHERE  (`email` ='".mysqli_real_escape_string($con,$user)."' or username='".mysqli_real_escape_string($con,$user)."') AND  `password` = '".mysqli_real_escape_string($con,$password)."' ";
    $REC = $DB->select($con,$sql);
    if (isset($REC[0][0])) {
        $REC = $REC[0];
        //Output		
        $XML = '<?xml version="1.0" encoding="utf-16"?>
			<List>
			 <userinfo>
				...
				<eamil>' . $REC['email'] . '</eamil>
                                <report> https://www.freelancenearme.com/dashboard/myproject_working</report>
                                <profile> https://www.freelancenearme.com/dashboard</profile>
			 </userinfo>
			</List>';
        echo $XML;
    }
        ...
        if (isset($REC[0][0])) {
            foreach ($REC as $R) {
                ...
                $XML.=
					...
					<project_by>' . "Employer: " . getEmployerName($con,$R['user_id']) . '</project_by>
...
if ($ACT == "prowork") {
    ...
    if ($user_id != "" && $project_id != "" && $type != "") {
        if ($type != "0") {
            ...
            if ($pwid != "") {
                $str_time = mysqli_fetch_assoc(mysqli_query($con,"select start_time from serv_project_tracker where project_id='" . $R_Q['pid'] . "' order by start_time desc limit 1"));
...
if ($ACT == "proworkstop") {
    ...
    if ($projectwork_id != "") {
		...
		$r = mysqli_fetch_assoc(mysqli_query($con,"select * from serv_project_tracker where id=".$projectwork_id));
